...
void Tokenizer::tokenize(std::string data) {
    ...
    while (!isEOF()) {
        ...
        while (isspace(lastChar)) { advance(1); }
        if (lastChar == '/') {
            if (peek(1) == '*') {
                advance(2);
                while (!isEOF()) {
            ...
            pushChar();
            if (isalpha(lastChar)) {
                while (!isEOF() && isalpha(lastChar)) { pushChar(); }
            ...
            while (isalpha(lastChar) || isdigit(lastChar) || lastChar == '_') { pushChar(); }
            ...
            if (lastChar == '0' && peek(1) == 'x') {
                ...
                for (; !isEOF() && !isspace(lastChar); i++) {
                    if (i > 1 && !isxdigit(lastChar)) nxne = true;
                    pushChar();
                }
                if (i > 10 || i < 4) werror.syntaxError(E_OUT_OF_RANGE_HEX, "Out Of Range Hex", "here", index);
                if (nxne) werror.syntaxError(E_INVALID_HEX, "Invalid Hex", "here", index);
                lastToken.kind = T_HEX;
            } else if (lastChar == '0' && peek(1) == 'b') {
                bool nbne = false;
                size_t i = 0;
                for (; !isEOF() && !isspace(lastChar); i++) {
                    if (i > 1 && lastChar != '0' && lastChar != '1' && !nbne) nbne = true;
                    pushChar();
                }
                if (i > 10) werror.syntaxError(E_OUT_OF_RANGE_BIN, "Out Of Range Binary", "here", index);
                if (nbne || i < 3) werror.syntaxError(E_INVALID_BIN, "Invalid Binary", "here", index);
                lastToken.kind = T_BIN;
            } else if (lastChar == '0' && peek(1) == 'u') {
                bool nune = false;
                size_t i = 0;
                for (; !isEOF() && !isspace(lastChar); i++) {
                    if (i > 1 && !isxdigit(lastChar) && !nune) nune = true;
                    pushChar();
                }
                if (i != 10 || nune) werror.syntaxError(E_INVALID_UNICODE, "Invalid Unicode", "here", index);
